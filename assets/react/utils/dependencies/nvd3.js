    ...
    selection.each(function(data) {
      ...
      switch (axis.orient()) {
        ...
        case 'bottom':
          ...
          axisLabel.enter().append('text').attr('class', 'nv-axislabel');
          var w = (scale.range().length==2) ? scale.range()[1] : (scale.range()[scale.range().length-1]+(0-scale.range()[0]));
          //console.log('nvtest',w,scale.range()[1],scale.range()[scale.range().length-1],scale.range()[1],scale.range()[0]);
          axisLabel
              .attr('text-anchor', 'middle')
              .attr('y', xLabelMargin)
              .attr('x', w/2);
